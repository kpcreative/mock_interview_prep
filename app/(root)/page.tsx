...
//as promise.all will allow them to fetch in parallel as it doesnt depend on each other...
 ...
 const [userInterviews,LatestInterview]=await Promise.all([
  ...
  return (
   <>
   <section className="card-cta bg-white text-black shadow-md">
    <div className="flex flex-col gap-6 max-w-lg">
       ...
       <Button asChild className='btn-primary max-sm:w-full bg-gray-800 hover:bg-gray-900 text-white shadow font-bold'>
        <Link href="/interview">
   ...
   <section className="flex flex-col gap-6 mt-8">
       <h2 className="text-black">Your Interview Is Ready – Take It to Help Us Improve</h2>
       <div className='interviews-section'>
        ...
        hasPastInterviews?(userInterviews?.map((interview)=>(
            <InterviewCard {...interview} key={interview.id}/>
        ))):(<p className="text-black">You have not taken any interviews yet</p>)
      }
       </div>
   </section>
   <section className='flex flex-col gap-6 mt-8'>
       <h2 className="text-black">This Is a Preview of an Interview Created by Others – Feedback Is Non-Official</h2>
       <div className='interviews-section'>
         {
        hasUpcomingInterviews?(LatestInterview?.map((interview)=>(
            <InterviewCard {...interview} key={interview.id}/>
        ))):(<p className="text-black">No any Taken interviews Yet by Any other users</p>)
      }
